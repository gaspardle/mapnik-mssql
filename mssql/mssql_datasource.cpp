...
std::string mssql_datasource::populate_tokens(std::string const& sql) const
{
    ...
    {
        box2d<double> max_env(-1.0 * (FMAX-1), -1.0 * (FMAX-1), (FMAX-1), (FMAX-1));
...
boost::shared_ptr<IResultSet> mssql_datasource::get_resultset(boost::shared_ptr<Connection> const &conn, std::string const& sql) const
{
...
featureset_ptr mssql_datasource::features(const query& q) const
{
#ifdef MAPNIK_STATS
    mapnik::progress_timer __stats__(std::clog, "mssql_datasource::features");
#endif
